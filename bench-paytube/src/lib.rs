...
use {
    ...
    std::sync::Arc,
    transaction::create_svm_transactions_original,
    transaction::create_svm_transactions_preallocated,
};
...
impl PayTubeChannel {
    ...
    pub fn process_paytube_transfers(
        ...
        let svm_transactions = if use_prealloc {
            create_svm_transactions_preallocated(transactions)
        } else {
            create_svm_transactions_original(transactions)
        };
